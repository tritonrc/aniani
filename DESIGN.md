...
The Traces view renders a selected trace as a Jaeger-style waterfall built entirely client-side from the OTLP-shaped `GET /api/traces/{traceID}` payload. Spans are flattened across the per-service `batches`, linked into a parent/child tree by `spanId`/`parentSpanId`, and laid out over a shared timeline: each span's bar offset and width are computed from its start/duration relative to the trace's earliest start and total span. Absolute nanosecond timestamps exceed JavaScript's safe-integer range, so they are parsed as `BigInt`; only the small intra-trace offsets and durations are narrowed to `Number` for layout. Spans are colored per service (a stable per-trace palette), the tree is collapsible, and clicking a span expands its tags, resource ("process") attributes, and event timeline — with OTLP exceptions (events named `exception`) surfaced as type, message, and stack trace. This is why the span model carries `kind` (OTLP `SpanKind`, for SERVER/CLIENT annotations) and `events` (timeline markers including exceptions), both decoded on OTLP ingest and emitted by the trace-detail endpoint.
